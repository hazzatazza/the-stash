...
let g_submitScore=(name,score)=>{};function pokiSubmitScore(levelID,score){g_submitScore(levelID,score)}
...
PokiSDK.init({submitScore:(submitScoreFromPoki)=>{g_submitScore=submitScoreFromPoki;}}).then(()=>{setPokiInited();}).catch(()=>{setPokiInited();});}
function PokiShowLeaderboard(leaderboardID){PokiSDK.showLeaderboard(leaderboardID);}
...
function pokiSendLevelData(){navigator.sendBeacon('https://leveldata.poki.io/fancade-drive_mad_2','f9564e4e-ef25-4e4b-ba67-cb11a1576bbd');}
...
async function performRequest(method,url,body,useToken,parseJson){url=encodeURI(url);simpleLogC(`performRequest(${method}, ${url}, ${useToken}, ${parseJson})`);let idToken=null;if(useToken){throw"error-no-user";}
return new Promise((resolve,reject)=>{var xhr=new XMLHttpRequest();xhr.responseType=parseJson?'text':'blob';xhr.onreadystatechange=function(){if(xhr.readyState===4){if(!(xhr.status>=200&&xhr.status<299)){simpleLogC("performRequest() status error "+xhr.status+", url: "+url);reject("status-"+xhr.status);}else{if(parseJson){try{const json=JSON.parse(xhr.response);if(json.error){reject(json.error.message);}else{resolve(json);}}catch(err){simpleLogC("performRequest() json parse error "+err+", response: "+xhr.response+", url: "+url);resolve(null);}}else{resolve(xhr.response);}}}};xhr.onerror=function(){simpleLogC("performRequest() XHR error url: "+url);reject("error-xhr");};xhr.open(method,url);if(useToken){xhr.setRequestHeader("Authorization",`Bearer ${idToken}`);}
if(!body)
body='';xhr.send(body);});}
function fetchUrl(url,id,useToken){let result=null;performRequest("GET",url,null,useToken,false).then((x)=>{result=x;}).finally(()=>{if(result===null){Module._web_command_fetch_send_url_done(0,0,id);}else{result.arrayBuffer().then((array)=>{const bytes=new Uint8Array(array);const data_size=bytes.length;const data_ptr=_malloc(data_size);Module.HEAPU8.set(bytes,data_ptr);Module._web_command_fetch_send_url_done(data_ptr,data_size,id);});}});}
function sendUrl(url,id,dataPtr,dataSize,useToken){const bytes=HEAPU8.subarray(dataPtr,dataPtr+dataSize);performRequest("PUT",url,bytes,useToken,false).finally(()=>{Module._web_command_fetch_send_url_done(0,0,id);});}
...
function adInterstitialShow(){simpleLogC("adInterstitialShow()");if(typeof PokiSDK!=="undefined"){PokiSDK.commercialBreak().then(()=>{setGameFocus(true);Module.ccall("ad_interstitial_on_showed","v",["number"],[1]);});}
else{Module.ccall("ad_interstitial_on_showed","v",["number"],[1]);}}
function adRewardedShow(){simpleLogC("adRewardedShow()");if(typeof PokiSDK!=="undefined"){PokiSDK.rewardedBreak().then((success)=>{Module.ccall("ad_rewarded_on_showed","v",["number"],[success?1:0]);if(success){Module.ccall("ad_rewarded_on_rewarded","v");}});}
else{Module.ccall("ad_rewarded_on_showed","v",["number"],[1]);Module.ccall("ad_rewarded_on_rewarded","v");}}
